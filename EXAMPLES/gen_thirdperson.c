...
char* hName = "Standalone Third Person";
char* hAuthor = "screen_names, HackerSmacker (this generator)";
char* hDesc = "Enable third person";
char* hVers = "1.0";
...
int main() {
    ...
    /* Generate modheader(s) */
    modheader = header(hName, hAuthor, hDesc, hVers);
    fwrite(modheader, sizeof(char), strlen(modheader), outFile);
    modheader = header_acontact("Discord", "screen_names");
    fwrite(modheader, sizeof(char), strlen(modheader), outFile);
    modheader = header_category("qol, resource, gameplay, bugfix");
    fwrite(modheader, sizeof(char), strlen(modheader), outFile);
    ...
    currentLine = hotfix(MOD_TYPE_PATCH, tpcameraobj, "LocationOffset", xyz, 0);
    fwrite(currentLine, sizeof(char), strlen(currentLine), outFile);
    /* Set the third person camera to be the default camera mode. */
    currentLine = hotfix(MOD_TYPE_PATCH, tpcameramode, "ModeName", "Default", 0);
    fwrite(currentLine, sizeof(char), strlen(currentLine), outFile);
    /* Change the default camera mode to actually be the third person camera. */
    currentLine = hotfix(MOD_TYPE_PATCH, defcameramode, "ModeName", "ThirdPerson", 0);
    fwrite(currentLine, sizeof(char), strlen(currentLine), outFile);
